    ...
    function readdir(directory, settings, callback) {
      settings.fs.readdir(directory, (readdirError, names) => {
        ...
        const tasks = names.map((name) => {
          const path5 = common2.joinPathSegments(directory, name, settings.pathSegmentSeparator);
          return (done) => {
...
var require_pg_connection_string = __commonJS({
  "../../node_modules/.pnpm/pg-connection-string@2.11.0/node_modules/pg-connection-string/index.js"(exports2, module2) {
    ...
    function parse(str2, options = {}) {
      ...
      const fs4 = config.sslcert || config.sslkey || config.sslrootcert ? require("fs") : null;
      if (config.sslcert) {
...
var ExpectationSchema = external_exports.object({
  ...
  json: external_exports.any().optional(),
  ...
  stdout_not_contains: external_exports.string().optional(),
  stderr_not_contains: external_exports.string().optional(),
  stdout_regex: external_exports.string().optional(),
  stderr_regex: external_exports.string().optional(),
  stdout_empty: external_exports.boolean().optional(),
  stderr_empty: external_exports.boolean().optional(),
  max_duration_ms: external_exports.number().optional(),
  rows: external_exports.number().optional(),
  equals: external_exports.record(external_exports.string(), external_exports.any()).optional()
});
var HttpBodySchema = external_exports.object({
  json: external_exports.any().optional(),
  jsonFile: external_exports.string().optional(),
...
var HttpStepSchema = external_exports.object({
  ...
  query: external_exports.record(external_exports.string(), external_exports.union([external_exports.string(), external_exports.number(), external_exports.boolean()])).optional(),
  /**
   * Back-compat + UX:
   * - body: { json: ... }      (preferred)
...
var ExecStepSchema = external_exports.object({
  run: external_exports.string(),
  cwd: external_exports.string().optional(),
  env: external_exports.record(external_exports.string(), external_exports.string()).optional(),
  timeout_ms: external_exports.number().optional(),
  retries: external_exports.number().optional(),
  max_output_chars: external_exports.number().optional()
});
var SqlStepSchema = external_exports.object({
  driver: external_exports.literal("postgres"),
  url_env: external_exports.string(),
  query: external_exports.string(),
  timeout_ms: external_exports.number().optional()
});
var StepSchema = external_exports.union([
  external_exports.object({ http: HttpStepSchema, expect: ExpectationSchema.optional() }),
  external_exports.object({ exec: ExecStepSchema, expect: ExpectationSchema.optional() }),
  external_exports.object({ sql: SqlStepSchema, expect: ExpectationSchema.optional() })
]);
var WorkItemObjectSchema = external_exports.object({
  key: external_exports.string(),
  says: external_exports.string().optional(),
  links: external_exports.array(external_exports.string()).optional()
});
var WorkItemSchema = external_exports.union([external_exports.string(), WorkItemObjectSchema]);
var CheckSchema = external_exports.object({
  ...
  says: external_exports.string().optional(),
  links: external_exports.array(external_exports.string()).optional(),
  steps: external_exports.array(StepSchema).min(1)
});
...
var SuiteSchema = external_exports.object({
  ...
  description: external_exports.string().optional(),
  contracts: external_exports.array(ContractSchema).min(1)
});

// src/contract.ts
function loadSuite(filePath) {
  const abs = import_node_path.default.resolve(process.cwd(), filePath);
  if (!import_node_fs.default.existsSync(abs)) throw new Error(`Contract file not found: ${abs}`);
  const raw = import_node_fs.default.readFileSync(abs, "utf8");
  const data = filePath.endsWith(".json") ? JSON.parse(raw) : jsYaml.load(raw);
...
function fakeValue(spec, rng) {
  const parts = spec.split(",").map((s) => s.trim());
  const name = parts[0];
  if (name === "uuid") return import_node_crypto.default.randomUUID();
  if (name === "email") {
    const n = randInt(rng, 1e3, 9999);
...
function resolveFakePlaceholders(value, seedKey) {
  const walk = (v, path5) => {
    if (typeof v === "string") {
      const m = v.match(/^fake\((.+)\)$/);
      if (!m) return v;
      const localSeed = hashToUInt32(`${seedKey}|${path5}|${m[1]}`);
...
function looksLikeBodyObject(x) {
  if (!isObj3(x)) return false;
...
async function runHttpStep(baseUrl, step, expect) {
  ...
  if (step.body !== void 0 && step.body !== null) {
    if (looksLikeBodyObject(step.body)) {
      if (typeof step.body.text === "string") {
        ...
        const raw = import_node_fs2.default.readFileSync(abs, "utf8");
        let parsed;
        try {
  ...
  const retries = typeof step.retries === "number" && Number.isFinite(step.retries) && step.retries >= 0 ? step.retries : envNum("STONEY_RETRIES", 2);
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (exp.json !== void 0) {
        ...
        } else if (!deepSubsetMatch(json2, exp.json)) {
          ok = false;
...
async function runExecStep(step, expect) {
  const notes = [];
  const title = `exec ${step.run}`;
  const timeoutMs = typeof step.timeout_ms === "number" ? step.timeout_ms : Number(process.env.STONEY_TIMEOUT_MS || 15e3);
  const retries = typeof step.retries === "number" ? step.retries : 0;
  const cwd = step.cwd ? import_node_path3.default.resolve(process.cwd(), step.cwd) : process.cwd();
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      const res = await new Promise((resolve, reject) => {
        ...
        const child = (0, import_node_child_process.spawn)(step.run, {
          cwd,
          shell: true,
        ...
        let stderr2 = "";
        let timedOut = false;
        child.stdout?.on("data", (d) => stdout2 += d.toString());
        child.stderr?.on("data", (d) => stderr2 += d.toString());
        const t = setTimeout(() => {
          timedOut = true;
          try {
            child.kill("SIGTERM");
        ...
        child.on("close", (code, signal) => {
          ...
          resolve({ code, signal, stdout: stdout2, stderr: stderr2, durationMs, timedOut });
        });
      });
      ...
      const maxDuration = expect?.max_duration_ms;
      ...
      const stderrRegex = expect?.stderr_regex;
